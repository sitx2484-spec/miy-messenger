...
function saveData() {
  try {
    const obj = {
      ...
      tubeVideos:  Object.fromEntries([...tubeVideos.entries()].map(([id,v])=>[id,{...v,videoData:undefined}])), // don't save video data (too large)
      tubeSeq,
    };
...
function tubePublic(v){return{id:v.id,title:v.title,description:v.description,thumbnail:v.thumbnail||'',authorId:v.authorId,authorName:v.authorName,authorAvatar:v.authorAvatar||'',views:v.views||0,likes:(v.likes||[]).length,liked:false,createdAt:v.createdAt,duration:v.duration||0,tags:v.tags||[]};}

app.get('/api/tube/videos',(req,res)=>{
  const{q,authorId}=req.query;
  let vids=[...tubeVideos.values()].filter(v=>!v.deleted);
  if(q){const ql=q.toLowerCase();vids=vids.filter(v=>(v.title||'').toLowerCase().includes(ql)||(v.description||'').toLowerCase().includes(ql));}
...
app.post('/api/tube/videos',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const{title,description,videoData,thumbnail,tags,duration}=req.body;
  ...
  const v={id,title:sanitize(title,200),description:sanitize(description||'',2000),videoData,thumbnail:thumbnail||'',authorId:u.id,authorName:prof.displayName||u.name,authorAvatar:prof.avatar||u.avatar||'',views:0,likes:[],tags:(tags||[]).slice(0,10),duration:duration||0,createdAt:Date.now(),deleted:false};
...
app.get('/api/tube/stats/:uid',(req,res)=>{
  ...
  res.json({videos:vids.length,views:vids.reduce((s,v)=>s+(v.views||0),0),likes:vids.reduce((s,v)=>s+(v.likes?.length||0),0)});
});
