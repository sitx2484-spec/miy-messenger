...
// On Render — use /var/data (persistent disk) if available, else local
...
const DATA_FILE = path.join(DATA_DIR, 'echo_data.json');
...
async function sendMail(to, subject, html) {
  ...
  if (!to || to.endsWith('@local')) return false;
  try {
    await Promise.race([
      mailer.sendMail({
...
// ── Data Encryption ───────────────────────────────────────────
const ENC_KEY = crypto.createHash('sha256').update(process.env.ENCRYPTION_KEY||SESSION_SECRET).digest();
...
function loadData() {
  const tryFiles = [
    DATA_FILE,
    DATA_FILE.replace('echo_data.json','echo_data_bak.json'),
    path.join(__dirname, 'data.json'), // legacy
  ];
  for(const f of tryFiles){
    try{
      if(fs.existsSync(f)){
        ...
        if(raw&&raw.length>2){
          const d=decryptLoad(raw);
          console.log(`✅ Data loaded from: ${f}`);
          return d;
        }
      }
    }catch(e){console.error(`⚠️ Cannot read ${f}:`,e.message);}
  }
...
function saveData() {
  try {
    const obj = {
      ...
      tubeVideos:  Object.fromEntries([...tubeVideos.entries()].map(([id,v])=>[id,{...v,videoData:undefined}])),
      tubeSeq,
      tubeChannels: Object.fromEntries(tubeChannels),
    };
    fs.writeFileSync(DATA_FILE, encryptSave(obj));
...
setInterval(()=>{
  try {
    if(fs.existsSync(DATA_FILE)){
      fs.copyFileSync(DATA_FILE, DATA_FILE.replace('echo_data','echo_data_bak'));
    }
  }catch{}
}, 5*60*1000);

// Save on shutdown (important for Render)
process.on('SIGTERM',()=>{console.log('SIGTERM — saving...');saveData();process.exit(0);});
process.on('SIGINT', ()=>{console.log('SIGINT — saving...');saveData();process.exit(0);});
...
const bruteForce = new Map(); // ip:key -> {fails, blockedUntil}

function getRateLimit(ip, max=60, windowMs=60000) {
  const now = Date.now();
  const key = ip;
...
function checkBruteForce(ip, identifier, maxFails=5, blockMs=15*60*1000) {
  const key = ip+':'+identifier;
  const now = Date.now();
  const b = bruteForce.get(key)||{fails:0, blockedUntil:0};
  if (b.blockedUntil > now) return {blocked:true, remainingMs: b.blockedUntil-now};
  return {blocked:false, fails:b.fails};
}
function recordFail(ip, identifier, maxFails=5, blockMs=15*60*1000) {
  const key = ip+':'+identifier;
  const now = Date.now();
  const b = bruteForce.get(key)||{fails:0, blockedUntil:0};
  b.fails++;
  if (b.fails >= maxFails) b.blockedUntil = now+blockMs;
  bruteForce.set(key, b);
}
function clearFails(ip, identifier) {
  bruteForce.delete(ip+':'+identifier);
}
...
function authRateLimit(req,res,next) {
  ...
  if (getRateLimit('auth:'+ip, 10, 60000)) {
    return res.status(429).json({error:'Занадто багато спроб входу. Зачекай 1 хвилину.'});
  }
  next();
}

// Bot detection middleware for public endpoints
function botProtect(req,res,next) {
  const ua = req.headers['user-agent']||'';
  const bots = ['curl','wget','python-requests','scrapy','bot','spider','crawl','axios','java','go-http','ruby','php'];
...
// Validate WS messages (size + type whitelist)
const WS_MAX_SIZE = 30 * 1024 * 1024; // 30MB
const WS_ALLOWED_TYPES = new Set([
  'auth','chat','pm','group_msg','group_voice','voice_msg','file_msg',
  'emoji_msg','circle_msg','create_group','add_to_group','leave_group',
  'call_request','call_answer','call_end','offer','answer','cam_offer',
  'cam_answer','ice','group_call_join','group_call_leave','group_offer',
  'group_answer','group_ice','rc_started','rc_stopped','rc_mouse','rc_key',
  'set_status','gift','set_group_role','secret_msg','game_move','game_invite',
  'poll_send','delete_msg','edit_msg','typing','reaction','daily_bonus',
  'avatar_update','nick_update','admin_verify','ping','vip_prefs',
  'channel_chat','admin_event',
]);
...
setInterval(()=>{
  ...
  for(const [k,v] of bruteForce) if(v.blockedUntil<now&&v.fails===0) bruteForce.delete(k);
}, 5*60*1000);
...
async function getGeoInfo(ip) {
  try {
    ...
    const r = await fetch(`http://ip-api.com/json/${ip}?fields=country,city,status`, {signal: controller.signal});
...
app.get('/auth/google/callback',
  ...
  async (req,res) => {
    try {
      const u = req.user;
      if (!u) return res.redirect('/?error=auth');

      const isNew = !getProfile(u.id)?.username;

      // If no email or no mailer — skip verification, go directly
      if (!u.email || u.email.endsWith('@local') || !mailer) {
        markDeviceKnown(u.id, req);
        return res.redirect(isNew ? '/?setup=1' : '/');
      }

      // Generate code
      const code = makeCode();

      // Get geo with 3s timeout (don't block on this)
      const geo = await withTimeout(getGeoInfo(getIp(req)), 3000, {country:'', city:''});
      ...
      pendingCodes.set(code, {
        ...
        isNew,
        ip: getIp(req), device,
        country: geo.country, city: geo.city, time,
        expires: Date.now() + 10*60*1000,
      });

      // Save pending user to session
      req.session.pendingGoogleUser = {id:u.id, email:u.email, name:u.name};

      // Log out passport session (user must verify first)
      await new Promise(r => req.logout(r));

      // Save session before redirect
      await new Promise(r => req.session.save(r));

      // Send email with 8s timeout — if fails, log in directly
      const sent = await withTimeout(
        sendMail(u.email, '🔐 Підтвердження входу в Echo', emailVerifyTpl(code)),
        8000, false
      );

      if (!sent) {
        // Email failed or timed out — log in directly without verification
        console.warn('[Auth] Email failed for', u.email, '— logging in directly');
        req.session.pendingGoogleUser = null;
        await new Promise(r => req.login(u, r));
        await new Promise(r => req.session.save(r));
        markDeviceKnown(u.id, req);
        return res.redirect(isNew ? '/?setup=1' : '/');
      }

      // Email sent — redirect to verify page
      return res.redirect('/?google_verify=1');

    } catch(err) {
      console.error('[Google callback error]', err.message);
      // On any error — try to log in directly
      try {
        const u = req.user;
        if (u) {
          await new Promise(r => req.login(u, r));
...
// Verify google email code
app.post('/api/auth/verify-google', authRateLimit, async (req,res) => {
  const ip = getClientIp(req);
  const {code} = req.body;
  if (!code) return res.status(400).json({error:'Введи код'});

  // Brute force check
  const bf = checkBruteForce(ip, 'gverify');
  if (bf.blocked) return res.status(429).json({error:`Заблоковано. Зачекай ${Math.ceil(bf.remainingMs/60000)} хв`});

  const pending = pendingCodes.get(code);
  if (!pending || pending.type !== 'google_verify') {
    recordFail(ip, 'gverify');
    return res.status(400).json({error:'Невірний код. Перевір пошту.'});
  }
  if (Date.now() > pending.expires) {
    pendingCodes.delete(code);
    return res.status(400).json({error:'Код прострочений. Увійди через Google знову.'});
  }

  clearFails(ip, 'gverify');
  pendingCodes.delete(code);
  ...
  res.json({
    ok: true,
    needsSetup: pending.isNew,
    user: {
      ...u, ...p,
      isAdmin: isAdmin(u.id), coins: getCoins(u.id),
      vip: isVip(u.id), privilege: getPriv(u.id),
      needsSetup: pending.isNew, verified: verifiedSet.has(u.id),
    }
  });
});

// Resend google verify code
app.post('/api/auth/resend-google', authRateLimit, async (req,res) => {
  const pending = req.session.pendingGoogleUser;
  if (!pending?.email) return res.status(400).json({error:'Немає активної сесії Google'});

  // Find existing pending code for this user
  let found = false;
  for (const [code, p] of pendingCodes) {
    if (p.type === 'google_verify' && p.userId === pending.id) {
      // Send again
      await sendMail(pending.email, '🔐 Новий код для Echo', emailVerifyTpl(code));
      found = true; break;
    }
  }
  if (!found) return res.status(400).json({error:'Код не знайдено. Увійди через Google знову.'});
  res.json({ok: true});
});

// ── Local auth (nickname + password) ─────────────────────────
app.post('/api/auth/local', authRateLimit, async (req,res) => {
  const ip = getClientIp(req);
  const {nickname, password, email} = req.body;
  const name = sanitize(nickname||'', 32).trim();
  const pass = (password||'').trim();
  const userEmail = sanitize(email||'', 254).trim().toLowerCase();
  ...
  if (localUsers[key]) {
    // Check brute force
    const bf = checkBruteForce(ip, 'login:'+key);
    if (bf.blocked) {
      const min = Math.ceil(bf.remainingMs/60000);
      return res.status(429).json({error:`Акаунт тимчасово заблокований на ${min} хв через підозрілу активність`});
    }
    const {salt,hash,id,email:storedEmail} = localUsers[key];
    const h = crypto.pbkdf2Sync(pass,salt,1000,32,'sha256').toString('hex');
    if (h !== hash) {
      recordFail(ip, 'login:'+key);
      const bf2 = checkBruteForce(ip, 'login:'+key);
      const left = 5 - bf2.fails;
      return res.status(401).json({error:`Невірний пароль${left>0?' (ще '+left+' спроб)':'. Заблоковано на 15 хв'}`});
    }
    clearFails(ip, 'login:'+key);
    ...
    if (realEmail && !realEmail.endsWith('@local') && !isKnownDevice(id, req)) {
      const ip2 = getIp(req);
      const geo = await getGeoInfo(ip2);
      const device = getDeviceInfo(req);
      const time = new Date().toLocaleString('uk-UA',{timeZone:'Europe/Kyiv'});
...
app.get('/api/channels/:id/discussion', (req,res) => {
  const u = req.user||req.session?.localUser; if(!u) return res.status(401).end();
  const ch = channels.get(req.params.id);
  if (!ch||!ch.subscribers.has(u.id)) return res.status(403).end();
  res.json(channelChats.get('ch_'+req.params.id)||[]);
});
...
// ── Admin ─────────────────────────────────────────────────────
// ══ ECHOTUBE ═════════════════════════════════════════════════
const tubeVideos=new Map(Object.entries(D.tubeVideos||{}));
let tubeSeq=D.tubeSeq||1;
// tube channel data: authorId -> {verified, subscribers:[uid,...], description, banner}
const tubeChannels=new Map(Object.entries(D.tubeChannels||{}));

function getTubeChannel(uid){
  if(!tubeChannels.has(uid))tubeChannels.set(uid,{verified:false,subscribers:[],description:'',banner:''});
  return tubeChannels.get(uid);
}

function tubePublic(v){
  const ch=getTubeChannel(v.authorId);
  return{id:v.id,title:v.title,description:v.description,thumbnail:v.thumbnail||'',
    authorId:v.authorId,authorName:v.authorName,authorAvatar:v.authorAvatar||'',
    authorVerified:ch.verified||false,
    views:v.views||0,likes:(v.likes||[]).length,liked:false,
    createdAt:v.createdAt,duration:v.duration||0,tags:v.tags||[],
    subtitles:v.subtitles||null,
    quality:v.quality||'auto',
  };
}

app.get('/api/tube/videos',(req,res)=>{
  const{q,authorId}=req.query;
  let vids=[...tubeVideos.values()].filter(v=>!v.deleted);
  if(q){const ql=q.toLowerCase();vids=vids.filter(v=>(v.title||'').toLowerCase().includes(ql)||(v.description||'').toLowerCase().includes(ql)||(v.tags||[]).some(t=>t.toLowerCase().includes(ql)));}
  if(authorId)vids=vids.filter(v=>v.authorId===authorId);
  vids.sort((a,b)=>(b.views||0)-(a.views||0));
  const u=req.user||req.session?.localUser;
  res.json(vids.slice(0,50).map(v=>({...tubePublic(v),liked:u?((v.likes||[]).includes(u.id)):false})));
});

app.get('/api/tube/videos/:id',(req,res)=>{
  const v=tubeVideos.get(req.params.id);if(!v||v.deleted)return res.status(404).end();
  v.views=(v.views||0)+1;
  const u=req.user||req.session?.localUser;
  res.json({...tubePublic(v),videoData:v.videoData,liked:u?((v.likes||[]).includes(u.id)):false});
});

app.post('/api/tube/videos',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const{title,description,videoData,thumbnail,tags,duration,subtitles}=req.body;
  if(!title||!videoData)return res.status(400).json({error:'Назва і відео обов\'язкові'});
  const prof=getProfile(u.id)||{};
  const id='tv_'+(tubeSeq++);
  const v={id,title:sanitize(title,200),description:sanitize(description||'',2000),
    videoData,thumbnail:thumbnail||'',
    authorId:u.id,authorName:prof.displayName||u.name,authorAvatar:prof.avatar||u.avatar||'',
    views:0,likes:[],tags:(tags||[]).slice(0,10),duration:duration||0,
    subtitles:subtitles||null, // VTT format string or null
    createdAt:Date.now(),deleted:false};
  tubeVideos.set(id,v);saveData();
  broadcast({type:'tube_new_video',video:tubePublic(v)});
  res.json({ok:true,id,video:tubePublic(v)});
});

app.post('/api/tube/videos/:id/like',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const v=tubeVideos.get(req.params.id);if(!v)return res.status(404).end();
  if(!v.likes)v.likes=[];
  const i=v.likes.indexOf(u.id);
  if(i>=0)v.likes.splice(i,1);else v.likes.push(u.id);
  saveData();res.json({ok:true,likes:v.likes.length,liked:v.likes.includes(u.id)});
});

app.delete('/api/tube/videos/:id',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const v=tubeVideos.get(req.params.id);if(!v)return res.status(404).end();
  if(v.authorId!==u.id&&!isAdmin(u.id))return res.status(403).end();
  v.deleted=true;saveData();res.json({ok:true});
});

// Subscribe/Unsubscribe to tube channel
app.post('/api/tube/subscribe/:uid',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const ch=getTubeChannel(req.params.uid);
  const i=ch.subscribers.indexOf(u.id);
  if(i>=0)ch.subscribers.splice(i,1);else ch.subscribers.push(u.id);
  saveData();
  res.json({ok:true,subscribed:ch.subscribers.includes(u.id),subscribers:ch.subscribers.length});
});

// Get channel info
app.get('/api/tube/channel/:uid',(req,res)=>{
  const u=req.user||req.session?.localUser;
  const ch=getTubeChannel(req.params.uid);
  const prof=getProfile(req.params.uid)||{};
  res.json({
    uid:req.params.uid,
    displayName:prof.displayName||req.params.uid,
    avatar:prof.avatar||'',
    verified:ch.verified||false,
    subscribers:ch.subscribers.length,
    subscribed:u?ch.subscribers.includes(u.id):false,
    description:ch.description||'',
    banner:ch.banner||'',
  });
});

// Update channel info (own channel)
app.post('/api/tube/channel/update',(req,res)=>{
  const u=req.user||req.session?.localUser;if(!u)return res.status(401).end();
  const ch=getTubeChannel(u.id);
  if(req.body.description!==undefined)ch.description=sanitize(req.body.description,500);
  if(req.body.banner!==undefined)ch.banner=req.body.banner||'';
  saveData();res.json({ok:true});
});

app.get('/api/tube/stats/:uid',(req,res)=>{
  const vids=[...tubeVideos.values()].filter(v=>v.authorId===req.params.uid&&!v.deleted);
  const ch=getTubeChannel(req.params.uid);
  res.json({
    videos:vids.length,
    views:vids.reduce((s,v)=>s+(v.views||0),0),
    likes:vids.reduce((s,v)=>s+(v.likes?.length||0),0),
    subscribers:ch.subscribers.length,
    verified:ch.verified||false,
  });
});

// Admin: give/revoke tube verified badge
app.post('/api/admin/tube-verify',(req,res)=>{
  const me2=req.user||req.session?.localUser;if(!me2||!isAdmin(me2.id))return res.status(403).end();
  const{uid,revoke}=req.body;
  const ch=getTubeChannel(uid);
  ch.verified=!revoke;
  saveData();
  broadcast({type:'tube_channel_update',uid,verified:!revoke});
  logAdmin(me2.id,'tube_verify'+(revoke?'_revoke':''),uid);
  res.json({ok:true});
});

// Admin: give subscribers count (manually set)
app.post('/api/admin/tube-subscribers',(req,res)=>{
  const me2=req.user||req.session?.localUser;if(!me2||!isAdmin(me2.id))return res.status(403).end();
  const{uid,count}=req.body;
  const ch=getTubeChannel(uid);
  // Fill fake subscribers up to count
  const current=ch.subscribers.length;
  const target=Math.max(0,Math.min(999999,parseInt(count)||0));
  if(target>current){
    for(let i=current;i<target;i++)ch.subscribers.push('_fake_'+i);
  }else{
    ch.subscribers=ch.subscribers.slice(0,target);
  }
  saveData();
  res.json({ok:true,subscribers:ch.subscribers.length});
});

// Admin list all tube channels
app.get('/api/admin/tube-channels',(req,res)=>{
  const me2=req.user||req.session?.localUser;if(!me2||!isAdmin(me2.id))return res.status(403).end();
  const authors=new Set([...tubeVideos.values()].filter(v=>!v.deleted).map(v=>v.authorId));
  const result=[...authors].map(uid=>{
    const prof=getProfile(uid)||{};
    const ch=getTubeChannel(uid);
    const videos=[...tubeVideos.values()].filter(v=>v.authorId===uid&&!v.deleted).length;
    return{uid,displayName:prof.displayName||uid,avatar:prof.avatar||'',
      verified:ch.verified,subscribers:ch.subscribers.length,videos};
  });
  res.json(result);
});
...
// Emoji shop
// ── Stickers / Emoji catalog with rarities ───────────────────
// type: 'sticker' means it's a sticker emoji (only from cases)
const STICKER_CATALOG = [
  // Рарка (Common) — 70%
  {id:'stk_devil',   name:'Чортик',    type:'sticker', rarity:'common',  emoji:'😈', color:'#cc0000', grad:'radial-gradient(circle at 35% 35%,#ff9999,#cc0000)'},
  {id:'stk_love',    name:'Закоханий', type:'sticker', rarity:'common',  emoji:'😍', color:'#ff3366', grad:'radial-gradient(circle at 35% 35%,#ffb3c6,#ff3366)'},
  {id:'stk_smile',   name:'Усмішка',   type:'sticker', rarity:'common',  emoji:'😊', color:'#ff9900', grad:'radial-gradient(circle at 35% 35%,#ffe066,#ff9900)'},
  {id:'stk_fire',    name:'Вогонь',    type:'sticker', rarity:'common',  emoji:'🔥', color:'#ff4400', grad:'radial-gradient(circle at 35% 35%,#ffcc00,#ff4400)'},
  // Епічний (Epic) — 25% (70+25=95 so 30% of remaining)
  {id:'stk_ghost',   name:'Привид',    type:'sticker', rarity:'epic',    emoji:'👻', color:'#8855ff', grad:'radial-gradient(circle at 35% 35%,#e8e8ff,#8855ff)'},
  {id:'stk_ice',     name:'Крижаний',  type:'sticker', rarity:'epic',    emoji:'🧊', color:'#00ccff', grad:'radial-gradient(circle at 35% 35%,#ccffff,#00ccff)'},
  {id:'stk_alien',   name:'Прибулець', type:'sticker', rarity:'epic',    emoji:'👽', color:'#00bb44', grad:'radial-gradient(circle at 35% 35%,#aaffaa,#00bb44)'},
  // Легендарний (Legendary) — 5%
  {id:'stk_rainbow', name:'Веселка',   type:'sticker', rarity:'legendary',emoji:'🌈', color:'#ff0080', grad:'linear-gradient(135deg,#f00,#f80,#ff0,#0f0,#00f,#80f)'},
  {id:'stk_star',    name:'Зірка',     type:'sticker', rarity:'legendary',emoji:'⭐', color:'#ffaa00', grad:'radial-gradient(circle at 35% 35%,#fff566,#ffaa00)'},
];

const RARITY_CONFIG = {
  common:    {name:'Рарка',       color:'#78909c', glow:'rgba(120,144,156,.4)', prob:0.70},
  epic:      {name:'Епічний',     color:'#9c27b0', glow:'rgba(156,39,176,.5)', prob:0.25},
  legendary: {name:'Легендарний', color:'#ff9800', glow:'rgba(255,152,0,.6)',  prob:0.05},
};

// EMOJI_CATALOG alias for backwards compat
const EMOJI_CATALOG = STICKER_CATALOG;

function rollCase() {
  const r = Math.random();
  let rarity;
  if (r < 0.05) rarity = 'legendary';
  else if (r < 0.30) rarity = 'epic';
  else rarity = 'common';
  const pool = STICKER_CATALOG.filter(s => s.rarity === rarity);
  return pool[Math.floor(Math.random() * pool.length)];
}

// Shop — only shows stickers info, can't buy (cases only)
app.get('/api/shop', (req,res) => res.json(STICKER_CATALOG.map(s => ({...s, caseOnly:true}))));

// Disable direct purchase
app.post('/api/buy-emoji', (req,res) => {
  res.status(403).json({error:'Стікери можна отримати тільки з кейсів!'});
});

// Cases
const CASES = [
  {
    id:'common_case',   name:'Рарка',       icon:'📦', price:200,
    grad:'linear-gradient(135deg,#607d8b,#90a4ae)',
    description:'Шанс отримати: Рарка 70% / Епічний 25% / Легендарний 5%',
    contains:['common','epic','legendary'],
  },
  {
    id:'epic_case',     name:'Епічний кейс', icon:'💜', price:500,
    grad:'linear-gradient(135deg,#7b1fa2,#ce93d8)',
    description:'Підвищений шанс на Епічний! 40% / Леген 15%',
    bonusEpic:true,
  },
  {
    id:'legendary_case',name:'Легендарний', icon:'🔥', price:1000,
    grad:'linear-gradient(135deg,#e65100,#ffcc02)',
    description:'Гарантований Епічний+ ! Леген шанс 30%',
    bonusLegendary:true,
  },
];
...
app.post('/api/open-case', (req,res) => {
  const u = req.isAuthenticated() ? req.user : req.session?.localUser;
  if (!u) return res.status(401).end();
  const c2 = CASES.find(c => c.id === req.body.caseId);
  if (!c2) return res.status(404).end();
  if (getCoins(u.id) < c2.price) return res.status(400).json({error:`Потрібно ${c2.price} ✈️`});
  addCoins(u.id, -c2.price); saveData();

  // Roll with bonus odds
  let r = Math.random(), rarity, item;
  if (c2.bonusLegendary) {
    if (r < 0.30) rarity='legendary'; else if (r < 0.70) rarity='epic'; else rarity='common';
  } else if (c2.bonusEpic) {
    if (r < 0.15) rarity='legendary'; else if (r < 0.55) rarity='epic'; else rarity='common';
  } else {
    if (r < 0.05) rarity='legendary'; else if (r < 0.30) rarity='epic'; else rarity='common';
  }
  const pool = STICKER_CATALOG.filter(s => s.rarity === rarity);
  item = pool[Math.floor(Math.random() * pool.length)] || STICKER_CATALOG[0];

  res.json({ok:true, coins:getCoins(u.id), item, rarity, rarityInfo:RARITY_CONFIG[rarity]});
});
...
wss.on('connection', ws => {
  ...
  const wsIp = ws._socket?.remoteAddress||'?';

  ws.on('message', raw => {
    // Size check
    if(raw.length > WS_MAX_SIZE){ws.close(1009,'Message too large');return;}

    // Rate limit: max 60 messages per 10 seconds
    const now=Date.now();
    if(now-msgReset>10000){msgCount=0;msgReset=now;}
    msgCount++;
    if(msgCount>60){ws.send(JSON.stringify({type:'error',msg:'Занадто швидко! Зачекай.'}));return;}

    let msg; try{msg=JSON.parse(raw);}catch{return;}

    // Type whitelist
    if(!msg.type||!WS_ALLOWED_TYPES.has(msg.type))return;

    // Sanitize text fields
    if(msg.text)msg.text=sanitize(msg.text,4000);
    if(msg.name)msg.name=sanitize(msg.name,64);
    if(msg.type==='auth'){
    ...
    switch(msg.type){
      ...
      case 'channel_chat':{
        ...
        if (!ch || !ch.subscribers.has(userId)) return;
        const prof = getProfile(userId)||{};
        const fullFrom = {
          ...from,
          displayName: prof.displayName || from?.name || '',
          username: prof.username || '',
          avatar: prof.avatar || from?.avatar || '',
          vip: isVip(userId),
        };
        const pkt = {
          ...
          fileName: sanitize(fileName||'', 256),
          fileType: (fileType||'').slice(0,100),
          time: Date.now(),
        };
        // Save to channel chat history (last 200 messages)
        const chatKey = 'ch_'+channelId;
        if (!channelChats.has(chatKey)) channelChats.set(chatKey, []);
        const hist = channelChats.get(chatKey);
        hist.push({...pkt, type:undefined}); // don't store type
      ...
      case 'vip_prefs':{
        if(!userId||!isVip(userId))return;
        const prefs={bg:sanitize(msg.bg||'',50)};
        vipPrefs.set(userId,prefs);
        saveData();
        // Broadcast to all so others see nick color change
        broadcast({type:'vip_prefs_update',userId,vipPrefs:prefs});
        break;
      }
      case 'ping':break;
      case 'admin_event':{
