...
console.log('🔑 GOOGLE_CLIENT_ID:', GOOGLE_ID ? '✅ є' : '❌ НЕМАЄ');
console.log('🔑 GOOGLE_CLIENT_SECRET:', GOOGLE_SECRET ? '✅ є' : '❌ НЕМАЄ');
console.log('🌐 BASE_URL:', process.env.BASE_URL || '❌ не задано');
...
app.get('/auth/google/callback',
  ...
  async (req,res) => {
    ...
    if (!u?.email || u.email.endsWith('@local')) {
      // No email — skip verification, go directly
      res.redirect(!getProfile(u.id)?.username ? '/?setup=1' : '/');
      return;
    }
    ...
    // Store pending with google user info in session temporarily
    ...
    // Save google user to session temporarily (not logged in yet)
    req.session.pendingGoogleUser = {id:u.id, email:u.email, name:u.name};
    req.logout(()=>{}); // log out from passport until code confirmed

    // Send code email
    const sent = await sendMail(u.email,
      '🔐 Підтвердження входу в Echo',
      emailVerifyTpl(code)
    );

    if (!sent) {
      // If email failed — just log in directly
